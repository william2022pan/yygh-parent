pipeline {
  ...
  stages {
    stage('拉取代码') {
      ...
      steps {
        container('maven') {
          git(url: 'https://gitee.com/leifengyang/yygh-parent.git', branch: 'master', credentialsId: 'gitee-id', changelog: true, poll: false)
          sh 'ls -al'
        }
    ...
    stage('发送确认邮件') {
      agent none
      steps {
        mail(to: 'anger_god@163.com', subject: '构建结果', body: "构建成功了 $BUILD_NUMBER")
      }
    }
